 ...
 */
function getJobs($conn, $tagId, $offset) {
...
/**
 * The function of the sign in to insert a new user
 * @param $conn: the connection to the database
...
function insertUser($conn, $email, $username, $password){
    ...
    if(usernameChecker($username, 1) || usernameChecker($username, 2) || usernameChecker($username, 3)){
        header('Content-Type: application/json');
    ...
    if ($stmt->execute()) {
        ...
        header('Content-Type: application/json');
        echo json_encode(['status' => 'error', 'message' => 'Something went wrong. Try another username or email']);
    }
...
function getJobById($conn, $jobId) {
    ...
    $sqlJob = "SELECT jobs.project_name AS title, jobs.project_description AS description, 
                      jobs.license, jobs.likes, jobs.layer_thickness, jobs.creation_date, 
                      jobs.color, users.username AS owner
    ...
    $stmtOtherJobs->bind_param("iii", $jobId, $jobId, $jobId);
...
error_log("Mensaje de prueba de error", 3, "/tmp/php_custom_error.log");
...
function setNewJob($conn, $username, $customer_name, $name, $desc, $license, $layer_thickness, $img_format, $scale, $color, $material, $tags, $files) {
    try {
        ...
        if (empty($name) || empty($layer_thickness) || empty($color)) {
            echo json_encode(['success' => false, 'message' => 'Missing required fields: name, layer thickness, or color']);
            return;
        }
        ...
        if (!in_array(strtolower($img_format), $allowed_formats)) {
            echo json_encode(['success' => false, 'message' => 'Incorrect image format']);
            return;
        }
        ...
        if ($stmt->execute()) {
            ...
            $fileStmt = $conn->prepare("INSERT INTO files (job_id, file_path, color, scale, physical_weight, file_weight, material) VALUES (?, ?, ?, ?, ?, ?, ?)");

            foreach ($files as $index => $file) {
                // Generate file path
                $file_path = "job_{$job_id}_file_" . ($index + 1) . ".stl";
                ...
                $fileStmt->bind_param("issddds", $job_id, $file_path, $file_color, $file_scale, $physical_weight, $file_weight, $material);
                $fileStmt->execute();
            }
            $fileStmt->close();
